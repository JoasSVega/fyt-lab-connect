...
const NoticiaClosing: React.FC<NoticiaClosingProps> = ({ noticia }) => {
  return (
    <footer className="w-full noticia-page__closing mt-16 pt-12 pb-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-12 space-y-8">
        ...
        {noticia.principalInstitutions && noticia.principalInstitutions.length > 0 && (
          <section>
            <h3 className="flex items-center gap-2 text-lg font-bold mb-4 font-raleway">
              <Building2 className="w-5 h-5 noticia-page__closing-icon" />
              Instituciones Involucradas
            </h3>
            <ul className="space-y-2">
              {noticia.principalInstitutions.map((institution, idx) => (
                <li key={idx} className="flex items-start gap-3 font-inter text-gray-700">
                  <span className="w-2 h-2 rounded-full mt-2 flex-shrink-0 noticia-page__closing-dot" />
        ...
        {noticia.researchGroups && noticia.researchGroups.length > 0 && (
          <section>
            <h3 className="flex items-center gap-2 text-lg font-bold mb-4 font-raleway">
              <Users className="w-5 h-5 noticia-page__closing-icon" />
              Grupos de Investigación
            </h3>
            <ul className="space-y-2">
        ...
        {noticia.relatedLinks && noticia.relatedLinks.length > 0 && (
          <section>
            <h3 className="flex items-center gap-2 text-lg font-bold mb-4 font-raleway">
              <LinkIcon className="w-5 h-5 noticia-page__closing-icon" />
            ...
            <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
              {noticia.relatedLinks.map((link, idx) => (
                <a
                  key={idx}
                  href={link.url}
                  className="inline-flex items-center gap-2 px-4 py-2 border rounded-lg transition-all duration-250 text-sm font-inter font-medium hover:translate-y-[-2px] noticia-page__closing-link"
                >
                  {link.title}
