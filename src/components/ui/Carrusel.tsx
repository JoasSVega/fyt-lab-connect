...
const Carrusel: React.FC<CarruselProps> = ({ items, color = "#9B59B6", height = 320, showDescription = true, className, imageClassName }) => {
  ...
  <Carousel opts={{ align: "center", loop: true, containScroll: 'keepSnaps', slidesToScroll: 1 }} setApi={setEmblaApi} className={"w-full"}>
      <CarouselContent className="gap-x-4">
        {items.map((item, index) => (
          <CarouselItem key={index} className="px-2 md:px-4 basis-full md:basis-1/2 lg:basis-1/3">
            <Card style={{ borderColor: color }} className={`bg-white/90 border-2 shadow-xl hover:shadow-2xl hover:scale-[1.03] transition-all duration-300 overflow-hidden group h-full`}>
              ...
              <div className="p-6">
                ...
                {showDescription && item.description && (
                  <p className="text-sm font-inter text-gray-700 leading-relaxed text-left">{item.description}</p>
                )}
              </div>
            </Card>
          </CarouselItem>
    ...
    <div className="flex items-center justify-center mt-4 space-x-2">
      {items.map((_, i) => (
        <button
          ...
          className={`h-2 w-8 rounded-full transition-all ${selected === i ? 'scale-110' : 'bg-gray-300'}`}
