# 📚 Guía Rápida de Documentación

**Última actualización**: 7 de enero de 2026

---

## 🚀 Empezar Aquí

- ¿Eres **nuevo en el proyecto**? → Leer [docs/CONTRIBUTING.md](docs/CONTRIBUTING.md)
- ¿Quieres entender la **arquitectura**? → Leer [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)
- ¿Necesitas una **referencia rápida**? → Ir a [docs/index.md](docs/index.md)

---

## 📂 Estructura de Documentación

### Raíz (`/`)
```
README.md          ← Presentación general del proyecto
CHANGELOG.md       ← Historial de versiones y cambios
```

### Documentación General (`/docs/`)
```
ARCHITECTURE.md               ← Arquitectura y estructura del proyecto
CALCULATORS.md                ← Documentación de calculadores
CONTRIBUTING.md               ← Cómo contribuir al proyecto
DIAGNOSTIC_CALCULATORS.md     ← Calculadores de diagnóstico
DIVULGACION_PUBLICATION_WORKFLOW.md ← Flujo de publicación de artículos
GOVERNANCE.md                 ← Gobernanza del proyecto
IMAGES.md                     ← Sistema de gestión de imágenes
IMG_RESPONSIVE_DESIGN.md      ← Imágenes responsivas y hero
PROJECT_VISION.md             ← Visión y objetivos del proyecto
ROADMAP.md                    ← Plan de desarrollo futuro
SESION_RESUMEN_2026_01_07.md  ← Resumen de sesión actual
```

### Guías Prácticas (`/docs/guides/`)
```
COMPATIBILIDAD_NAVEGADORES.md     ← Safari, Chrome, Firefox
MEJORA_TIPOGRAFIA_PREMIUM.md      ← Diseño editorial premium
PORTAL_ACADEMICO_GUIDE.md         ← Implementación portal académico
PORTAL_ACADEMICO_RESUMEN.md       ← Resumen portal académico
SEO_ARCHITECTURE.md               ← Arquitectura SEO
SEO_IMPLEMENTATION_SUMMARY.md     ← Implementación SEO
SEO_QUICK_REFERENCE.md            ← Referencia rápida SEO
LOADER_SYSTEM_DOCS.md             ← Sistema de loaders
SMART_IMAGE_REFACTOR_GUIDE.md     ← Refactorización de imágenes
TEST_SEO_CHECKLIST.md             ← Checklist de pruebas
PR_CALCULATORS_UI.md              ← UI de calculadores
```

### Reportes y Auditorías (`/docs/reports/`)
```
SEO_AUDIT_2026.md                 ← Auditoría SEO completa
SECURITY_AUDIT_2026_01_06.md      ← Auditoría de seguridad
RELEASE_REPORT_2026_01_06.md      ← Release del 6 de enero
... (más de 20 reportes de optimización)
```

### Componentes (`/docs/components/`)
```
SMARTIMAGE.md                 ← Documentación del componente SmartImage
```

### Ejemplos (`/docs/examples/`)
```
PUBLICACIONES_REFACTORED.md   ← Ejemplo de refactorización
```

---

## 🎯 Buscar por Tema

**SEO**
- Referencia rápida: [SEO_QUICK_REFERENCE.md](docs/guides/SEO_QUICK_REFERENCE.md)
- Arquitectura: [SEO_ARCHITECTURE.md](docs/guides/SEO_ARCHITECTURE.md)
- Auditoría completa: [SEO_AUDIT_2026.md](docs/reports/SEO_AUDIT_2026.md)

**Imágenes**
- Sistema general: [IMAGES.md](docs/IMAGES.md)
- Diseño responsivo: [IMG_RESPONSIVE_DESIGN.md](docs/IMG_RESPONSIVE_DESIGN.md)
- Componente SmartImage: [SMARTIMAGE.md](docs/components/SMARTIMAGE.md)

**Calculadores**
- Documentación: [CALCULATORS.md](docs/CALCULATORS.md)
- Diagnóstico: [DIAGNOSTIC_CALCULATORS.md](docs/DIAGNOSTIC_CALCULATORS.md)
- UI/UX: [PR_CALCULATORS_UI.md](docs/guides/PR_CALCULATORS_UI.md)

**Portal Académico**
- Guía: [PORTAL_ACADEMICO_GUIDE.md](docs/guides/PORTAL_ACADEMICO_GUIDE.md)
- Resumen: [PORTAL_ACADEMICO_RESUMEN.md](docs/guides/PORTAL_ACADEMICO_RESUMEN.md)

**Compatibilidad y Navegadores**
- Guía: [COMPATIBILIDAD_NAVEGADORES.md](docs/guides/COMPATIBILIDAD_NAVEGADORES.md)

**Tipografía y Diseño**
- Upgrade premium: [MEJORA_TIPOGRAFIA_PREMIUM.md](docs/guides/MEJORA_TIPOGRAFIA_PREMIUM.md)

**Divulgación Científica**
- Workflow: [DIVULGACION_PUBLICATION_WORKFLOW.md](docs/DIVULGACION_PUBLICATION_WORKFLOW.md)

---

## 📖 Índices Completos

- **Índice maestro**: [docs/index.md](docs/index.md) ⭐ **EMPEZAR AQUÍ**
- **Resumen de reorganización**: [docs/RESUMEN_REORGANIZACION_MARKDOWN.md](docs/RESUMEN_REORGANIZACION_MARKDOWN.md)
- **Auditoría markdown**: [docs/reports/AUDIT_MARKDOWN_FILES.md](docs/reports/AUDIT_MARKDOWN_FILES.md)

---

## ✅ Estándares de Documentación

Todos nuestros archivos markdown siguen estos estándares:

1. **Nombre**: `UPPER_SNAKE_CASE.md` (ejemplo: `SEO_QUICK_REFERENCE.md`)
2. **Idioma**: 100% español (términos técnicos en inglés: SEO, CSS, API)
3. **Ubicación**: Organizado por propósito (guides, reports, components, examples)
4. **Contenido**: Claro, actualizado, con propósito específico

---

## 🔄 Últimos Cambios

**7 de enero de 2026**: Auditoría exhaustiva y reorganización de archivos markdown
- Reorganizados 50 archivos `.md`
- Movidos 14 archivos a ubicaciones apropiadas
- Renombrados 2 archivos para consistencia
- Índice maestro actualizado
- Documentación de auditoría generada

Ver: [RESUMEN_REORGANIZACION_MARKDOWN.md](docs/RESUMEN_REORGANIZACION_MARKDOWN.md)

---

## 💡 Tips Útiles

### Para Encontrar Información Rápido
1. Ir a [docs/index.md](docs/index.md)
2. Usar Ctrl+F para buscar por palabra clave
3. Revisar la sección "¿Necesitas..." para casos comunes

### Para Contribuir Documentación
1. Leer [docs/CONTRIBUTING.md](docs/CONTRIBUTING.md)
2. Seguir convención: `UPPER_SNAKE_CASE.md`
3. Escribir en español
4. Agregar enlace en [docs/index.md](docs/index.md)
5. Hacer commit descriptivo

### Para Reportar Documentación Faltante
1. Crear issue en GitHub
2. Etiquetar con `documentation`
3. Describir qué información falta

---

**¿Algo no está claro?** Revisar [docs/index.md](docs/index.md) o crear un issue.
